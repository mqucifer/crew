...
`crew tick` runs seven phases in dependency order until a pass moves nothing. One command takes the board as far as it can go.

```mermaid
flowchart TD
  S([pass starts]) --> RV[revisit]
  RV --> R[refine]
  R --> DN[design]
  DN --> A[admit]
  A --> V[review]
...
| refine | Inbox (Goals), Needs Refinement | Product Owner proposes epics; Business Analyst splits approved epics into stories. A project whose design is being revisited has its epics wait, except technical ones | new issues, cards to Inbox and Ready |
